...
//    file_ = context->getSubsystem<File>();
    ...
    std::string d_vs = "#version 300 es\n"
            ...
            "}";
    std::string d_fs = "#version 300 es\n"
...
//    LOGE("d_fs:%s",d_fs.c_str());
    ...
    std::string d_use_vs = "#version 300 es\n"
            "layout(location = 0)in vec3 positions;\n"
            ...
            "}";
    basicString d_use_fs = "#version 300 es\n"
            "precision highp float;\n"
            "precision highp int;\n"
            "out vec4 fragColor;\n"
            "in vec2 vTexCoords;\n"
...
//    LOGE("d_use_fs:%s",d_use_fs.c_str());
    ...
    image->loadImage("/sdcard/klin/face1.png");
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32F, width_, height_, 0, GL_DEPTH_COMPONENT,
                 GL_FLOAT, NULL);
