...
ShaderProgram::ShaderProgram(Graphics *graphics, ShaderVariation *vertexShader,
...
bool ShaderProgram::Link() {
    if (!vertexShader_ || !pixeShader_ || !vertexShader_->getGPUObjectName() ||
        !pixeShader_->getGPUObjectName())
        return false;
    //创建着色器
    ...
    for (int i = 0; i < attributeCount; i++) {
        glGetActiveAttrib(object_.name_, i, (GLsizei) MAX_NAME_LENGTH, &nameLength, &elementCount,
                          &type, nameBuffer);
    ...
    for (int i = 0; i < uniformCount; ++i) {
        glGetActiveUniform(object_.name_, (GLuint) i, MAX_NAME_LENGTH, nullptr, &elementCount,
                           &type, nameBuffer);
...
const ShaderParameter *ShaderProgram::GetParameter(string param) const {
    KL_Map<string, ShaderParameter>::const_iterator i = shaderParameters_.find(param);
